...
const LEGACY_BRIDGE_BASE = 'ipc://direct';
...
function bridgeHttpBase() {
    return apiModule ? apiModule.bridgeHttpBase(bridgePort) : 'ipc://direct';
}
...
function connectBridge(forceReconnect = false) {
    ...
    socket = window.WSHawkIPC.createEventSocket();
...
document.getElementById('reqforge-store-dom-identity')?.addEventListener('click', async () => {
    ...
    const inferredAlias = (() => {
        const username = normalized(
            ...
            || storage.auth_user
            || storage.username
            || flow.extracted_tokens?.auth_user
        ...
        const role = normalized(
            ...
            || storage.auth_role
            || storage.role
            || flow.extracted_tokens?.auth_role
    ...
    try {
        ...
        const res = await ipcRequest(`/platform/projects/${project.id}/identities`, {
            method: 'POST',
...
document.getElementById('reqforge-authz-matrix-btn')?.addEventListener('click', async () => {
    if (attacksModule?.runAuthzMatrix) await attacksModule.runAuthzMatrix(getAttackContext());
});
