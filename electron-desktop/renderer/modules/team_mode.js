(function TeamModeEngine() {
    ...
    const TeamClient = {
        ...
        async createRoom(operatorName, target) {
            const res = await ipcRequest(`${BRIDGE_URL}/team/create`, {
                method: 'POST',
        ...
        async validateRoom(roomCode) {
            const res = await ipcRequest(`${BRIDGE_URL}/team/join`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        async getStats() {
            const res = await ipcRequest(`${BRIDGE_URL}/team/stats`);
            return res.json();
        },
