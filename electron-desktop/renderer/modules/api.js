    ...
    modules.api = {
        ...
        bridgeWebSocketUrl(port, path) {
            ...
            if (normalized.startsWith('/proxy?')) {
                const target = new URL(`ipc://direct${normalized}`).searchParams.get('url');
                if (target) return target;
            }
            return `ipc://direct${normalized}`;
        },

        bridgeFetch(rawFetch, port, input, init = {}, legacyBase = 'ipc://direct') {
            let url = input;
        ...
        async ensurePlatformProject(ctx, reason = 'operation', targetUrlOverride = '') {
            ...
            ctx.platformState.syncPromise = (async () => {
                ...
                const res = await ipcRequest('/platform/projects', {
                    method: 'POST',
        ...
        async refreshPlatformProjectSummary(ctx, { silent = true } = {}) {
            ...
            try {
                const [projectRes, eventsRes, evidenceRes] = await Promise.all([
                    ipcRequest(`/platform/projects/${currentProject.projectId}`),
                    ipcRequest(`/platform/projects/${currentProject.projectId}/events?limit=200`),
                    ipcRequest(`/platform/projects/${currentProject.projectId}/evidence?limit=100`)
                ]);
