(function HighlightToHack() {
    ...
    ctxMenu.querySelectorAll('.ai-ctx-item').forEach(item => {
        item.addEventListener('click', async () => {
            ...
            try {
                const res = await ipcRequest('/ai/context-exploit', {
                    method: 'POST',
...
async function toggleInterceptor(enabled) {
    try {
        await ipcRequest('/interceptor/toggle', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function sendFrameAction(action) {
    ...
    try {
        await ipcRequest('/interceptor/action', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
if (payloadSelect) {
    payloadSelect.addEventListener('change', async (e) => {
        ...
        try {
            const res = await ipcRequest(`/blaster/payloads/${category}`);
            const data = await res.json();
...
blasterBtn.addEventListener('click', async () => {
    ...
    window.WSHawkDOM.emptyTable(document.getElementById('blaster-tbody'), 'Fuzzing...', 6);
    blasterBtn.disabled = true;
    blasterBtn.innerText = "BLASTING...";
    document.getElementById('blaster-stop-btn').style.display = 'block';

    if (domVerify) appendLog('info', '[DOM Invader] Browser verification will only confirm execution when an explicit application page and selector are available.');
    if (authFlow) appendLog('info', '[DOM Invader] Auth flow active — will auto-replay on session expiry.');

    try {
        const response = await ipcRequest('/blaster/start', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                url: u,
        ...
        if (!response.ok) throw new Error(result.detail || result.message || 'Payload blast failed');
    } catch (e) {
...
if (blasterStopBtn) {
    blasterStopBtn.addEventListener('click', async () => {
        try {
            await ipcRequest('/blaster/stop', { method: 'POST' });
...
(function DOMInvaderUI() {
    ...
    async function checkStatus() {
        try {
            const res = await ipcRequest('/dom/status');
            const data = await res.json();
            ...
            if (data.playwright_installed) {
                ...
                statusPill.title = 'Install Playwright Chromium or configure a supported local Chromium executable.';
            }
        } catch (_) {
            if (statusPill) {
        ...
        recordBtn.addEventListener('click', async () => {
            ...
            try {
                if (typeof window.prompt === 'function') {
                    ...
                    loginUrl = validLoginUrl(prompted);
                }
            } catch (_) {
                // Fall back to inferred URL when dialogs are unavailable in the shell.
            }

            loginUrl = validLoginUrl(loginUrl);
            if (!loginUrl) {
                if (authStatus) {
                    authStatus.style.display = 'block';
                    authStatus.textContent = 'Enter a complete HTTP(S) login URL, including its hostname and path.';
                }
                appendLog('vuln', '[DOM Invader] Auth recording requires a complete HTTP(S) login URL.');
                return;
            }
            ...
            try {
                ...
                const res = await ipcRequest('/dom/auth/record', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        login_url: loginUrl,
    ...
    if (replayBtn) {
        replayBtn.addEventListener('click', async () => {
            ...
            if (authStatus) {
                authStatus.style.display = 'block';
                authStatus.textContent = 'Replaying captured authentication in an isolated browser context...';
            }
            try {
                const response = await ipcRequest('/dom/auth/replay', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ url: flow.url, flow, timeout_ms: 30_000 }),
                });
                const data = await response.json();
                if (!response.ok || data.status !== 'success') throw new Error(data.detail || data.message || 'Authentication replay failed');
                if (authStatus) authStatus.textContent = `Replay reached ${data.replay.url} with HTTP ${data.replay.status}.`;
                appendLog('success', `[DOM Invader] Authentication replay completed with HTTP ${data.replay.status}.`);
            } catch (error) {
                if (authStatus) authStatus.textContent = `Replay failed: ${error.message}`;
                appendLog('vuln', `[DOM Invader] Authentication replay failed: ${error.message}`);
            } finally {
...
if (btnSettings && settingsModal) {
    ...
    btnSettings.addEventListener('click', async () => {
        ...
        try {
            const res = await ipcRequest('/config/get');
            const data = await res.json();
    ...
    if (btnSettingsSave) {
        btnSettingsSave.addEventListener('click', async () => {
            ...
            try {
                const res = await ipcRequest('/config/save', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
...
if (btnExtensionPairingCode && extensionPairingCode) {
    btnExtensionPairingCode.addEventListener('click', async () => {
        ...
        try {
            const response = await ipcRequest('/extension/pairing/approve', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
