    ...
    modules.protocol = {
        ...
        initWSMap(ctx = {}) {
            ...
            async function loadDiscovery() {
                ...
                try {
                    const res = await ipcRequest('/discovery/scan', {
                        method: 'POST',
            ...
            async function loadLiveMap({ allowProjectBootstrap = false } = {}) {
                ...
                try {
                    const res = await ipcRequest(`/platform/projects/${project.projectId}/protocol-map?limit=500`);
                    const data = await res.json().catch(() => ({}));
                    if (!res.ok) {
            ...
            async function handleProbeOrRefresh() {
                ...
                try {
                    const res = await ipcRequest('/discovery/probe', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
