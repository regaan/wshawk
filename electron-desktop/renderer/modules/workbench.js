(function initCodec() {
    ...
    const ops = {
        ...
        'hex-decode': (s) => new TextDecoder().decode(new Uint8Array(s.trim().split(/[\s,]+/).map(h => parseInt(h, 16)))),
    ...
    document.getElementById('codec-smart-btn').addEventListener('click', () => {
        ...
        while (maxIterations-- > 0) {
            ...
            if (!decoded && /^([0-9a-fA-F]{2}[\s,]*)+$/.test(data.trim())) {
                try {
                    const d = new TextDecoder().decode(new Uint8Array(data.trim().split(/[\s,]+/).map(h => parseInt(h, 16))));
	...
	document.getElementById('codec-binary-analyze-btn')?.addEventListener('click', async () => {
		const encoded = input.value.trim();
		if (!encoded) return;
		try {
			const response = await ipcRequest('/binary/analyze', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ payload_base64: encoded }),
			});
			const data = await response.json();
			if (!response.ok) throw new Error(data.detail || data.message || 'Binary analysis failed');
			output.value = JSON.stringify(data.analysis, null, 2);
			chainLabel.textContent = `${data.analysis.magic || 'unknown'} · ${data.analysis.size || 0} bytes`;
		} catch (error) {
			output.value = `[ERR] ${error.message}`;
		}
	});
...
(function initAuthBuilder() {
    ...
    document.getElementById('auth-test-btn').addEventListener('click', async () => {
        ...
        try {
            const res = await ipcRequest('/auth/test', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    function startSchedule(idx) {
        ...
        timers[idx] = setInterval(async () => {
            ...
            try {
                ...
                const res = await ipcRequest('/scan/start', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
...
(function initOAST() {
    ...
    pollBtn.addEventListener('click', async () => {
        try {
            const res = await ipcRequest('/oast/poll');
            const data = await res.json();
...
(function initMutationLab() {
    ...
    runBtn.addEventListener('click', async () => {
        ...
        try {
            const res = await ipcRequest('/mutate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
