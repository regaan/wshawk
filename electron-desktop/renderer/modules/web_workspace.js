(function initWebWorkspace(global) {
    ...
    async function exportProjectBundle(projectId) {
        const response = await ipcRequest(`/platform/projects/${projectId}/exports/json`);
        if (!response.ok) {
    ...
    async function renderBoard() {
        ...
        state.refreshPromise = (async () => {
            const [projectRes, targetsRes, flowsRes] = await Promise.all([
                ipcRequest(`/platform/projects/${projectId}`),
                ipcRequest(`/platform/projects/${projectId}/targets?limit=24`),
                ipcRequest(`/platform/projects/${projectId}/http-flows?limit=24`),
            ]);
