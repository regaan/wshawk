#!/usr/bin/env python3
"""
WSHawk Advanced CLI - modern interface for the WSHawkV2 scanner
"""
...
async def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        prog="wshawk-advanced",
        description=f"WSHawk v{__version__} - Advanced WebSocket Security Scanner",
...
Examples:
  ...
  wshawk-advanced --web --port 8080                  # Web GUI on custom port
    ...
    parser.add_argument("url", nargs="?", default=None, help="Target URL (ws://, wss://, http://, or https://)")
    parser.add_argument("--version", action="version", version=f"wshawk-advanced {__version__}")
    parser.add_argument("--playwright", action="store_true", help="Enable Playwright browser for XSS verification")
    parser.add_argument("--no-oast", action="store_true", help="Disable OAST (Out-of-Band) testing")
    parser.add_argument("--rate", type=int, default=10, help="Max requests per second (default: 10)")
    parser.add_argument("--full", action="store_true", help="Enable ALL features (Playwright + OAST + Session tests)")
    parser.add_argument(
        "--discover", action="store_true", help="Discover WebSocket endpoints from HTTP target before scanning"
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument("--output", "-o", type=str, default=None, help="Output file path for report")
    parser.add_argument("--binary", action="store_true", help="Enable binary WebSocket message analysis")
    ...
    integ.add_argument("--dd-product", type=int, default=None, help="DefectDojo product ID")
    integ.add_argument(
        "--jira",
        type=str,
        default=None,
    ...
    integ.add_argument("--jira-project", type=str, default="SEC", help="Jira project key (default: SEC)")
    integ.add_argument(
        "--webhook",
        type=str,
        default=None,
    ...
    integ.add_argument(
        "--webhook-platform",
        choices=["slack", "discord", "teams", "generic"],
        default="generic",
    ...
    smart.add_argument("--smart-payloads", action="store_true", help="Enable context-aware adaptive payload generation")

    # ─── Web GUI ────────────────────────────────────────────────
    web = parser.add_argument_group("Web GUI")
    web.add_argument("--web", action="store_true", help="Launch Flask-based Web GUI instead of CLI scan")
    web.add_argument("--host", type=str, default="127.0.0.1", help="Web GUI host (default: 127.0.0.1)")
    web.add_argument("--port", type=int, default=5000, help="Web GUI port (default: 5000)")
    ...
    if args.no_oast:
        config.set("scanner.features.oast", False)
    if args.binary:
        config.set("scanner.features.binary_analysis", True)
    if args.smart_payloads:
        config.set("scanner.features.smart_payloads", True)
    ...
    if args.web:
        ...
        run_web(host=host, port=port, auth_enabled=auth_enabled, auth_password=auth_password, db_path=clean_db_path)
        return
    ...
    Logger.info(f"WSHawk v{__version__} - Advanced Scanner")
